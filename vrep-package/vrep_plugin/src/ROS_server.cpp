...
bool ROS_server::launchPublisher(SPublisherData& pub,int queueSize)
{
	...
	// added by Yu Zhang for imu sensor
	if(pub.cmdID == simros_strmcmd_get_imu_state)
	{
		pub.generalPublisher=node->advertise<sensor_msgs::Imu>(pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
...
void ROS_server::streamAllData()
{
	...
	for (int pubI=0;pubI<int(publishers.size());pubI++)
	{
		...
		{ // we handle only the publishers that are not asleep!
			...
			// added by Yu Zhang for imu sensor
			if(publishers[pubI].cmdID == simros_strmcmd_get_imu_state)
			{
				sensor_msgs::Imu fl;//sensor msgs to be published
				int imu_Handle = publishers[pubI].auxInt1;  // get the handle of imu object
				simChar* imu_name = simGetObjectName(imu_Handle);  // get the name of imu object
				if ( imu_name != NULL)
				{
					//ok ,the imu object exists
					std::string imu_frame_id = (imu_name);
					simReleaseBuffer(imu_name);
					imu_frame_id = objNameToFrameId(imu_name);
					int header_Handle = publishers[pubI].auxInt2;
					if (header_Handle == sim_handle_parent)
						header_Handle = simGetObjectParent(imu_Handle);
					std::string header_frame_id = "world";
					if(header_Handle != -1)
					{
						simChar* pname = simGetObjectName(header_Handle);
					...
					if( simGetObjectQuaternion(imu_Handle, header_Handle, q) == -1)
					{
						q[0] = 0;
						q[1] = 0;
						q[2] = 0;
					...
					fl.header.seq = _simulationFrameID;
					fl.header.stamp = inf.headerInfo.stamp;
					fl.header.frame_id = "1";  //1: global frame  0:no frame

					// get the string singal data
					float val[6];
					int dataLength = 0;
					std::string stringSignalName = publishers[pubI].auxStr; // get the name of string signal
					const char* data=simGetStringSignal(stringSignalName.c_str(),&dataLength);
					if(!data || (dataLength != 6*sizeof(float))) //check the data
					{
						removeThisPublisher = true;
					}
					else
					{
						...
						simReleaseBuffer((simChar*)data);
						fl.orientation.x = (double)q[0];
						fl.orientation.y = (double)q[1];
						fl.orientation.z = (double)q[2];
						fl.orientation.w = (double)q[3];
						fl.linear_acceleration.x =(double)val[0];
						fl.linear_acceleration.y =(double)val[1];
						fl.linear_acceleration.z =(double)val[2];
						fl.angular_velocity.x = (double)val[3];
						fl.angular_velocity.y = (double)val[4];
						fl.angular_velocity.z = (double)val[5];
						publishedSomething = true;
						publishers[pubI].generalPublisher.publish(fl);
					}
				}
